<?php
// Tham khao them Yokohama Wright
session_start();
...
//email list
$aMailto = array("vntestoutsource@gmail.com", "vntestoutsource@gmail.com", "vntestoutsource2@gmail.com");
...
$filePath = dirname(__FILE__).'/attachment-file/';
$linkFolder = 'http://'.$_SERVER[HTTP_HOST].'/yokohama-wright/estimate/attachment-file/';
$linkDir = '/estimate/attachment-file/';

//echo $linkFolder; die();

//======================================================================================== F1
$reg_txt_file1 = htmlspecialchars($_POST['file_txt01']);
$reg_file1 = htmlspecialchars($_POST['file1']);
$getfile1 = $_POST['getfile1'];
if(isset($_FILES['file1']['name'])) {
	if ($_FILES['file1']['size'] > 5024288){
		$messagesize_01 = 'File too large. File must be less than 5 megabytes.';
	}
	elseif ($_FILES['file1']['name'] != '') {
		$parts1=pathinfo($_FILES['file1']['name']);
		$ext1=".".strtolower($parts1["extension"]);
		$filename1 = strtolower($parts1["filename"]);
		if (file_exists($_SERVER['DOCUMENT_ROOT'].$linkDir.$_FILES["file1"]["name"])) {
			$attach_file1 = "file1_".$filename1."_".date("sihdmy")."_".$ext1;
			$_SESSION["fileurl1"] = $attach_file1;
		} else {
			$attach_file1 = str_replace(" ","_",$_FILES['file1']['name']);
			$_SESSION["fileurl1"] = $attach_file1;
		}
		// print_r($_SESSION["fileurl1"]); die();
		// echo $filePath.$attach_file1; die();
		if(move_uploaded_file($_FILES['file1']['tmp_name'], $filePath.$attach_file1)) {
			$linkpdf01= $linkFolder.$attach_file1;
		}
	}
}
...
if(isset($getfile1) && $getfile1 != 'http://inspect.alive-web.info/yokohama-wright/estimate/attachment-file/')
{
$msgBody .= "
■図面の添付
$getfile1
...
<script type="text/javascript">
	$(function(){
		jQuery.preloadFile = function(){
			$("#filetrung1").val('<?php echo $_SESSION['fileurl1'] ?>');
		};
		$.preloadFile();
	});
</script>
...
<script type="text/javascript">
	$(function(){
		...
		$("input:file").change(function (){
		    ...
		    var regex = new RegExp("(.*?)\.(png|jpg|jpeg|gif|pdf)$");
		    if(!(regex.test(val)) || fileSize > 1) {
		      $(this).val('');
					span.text('Error. Please try again.');
		    } else {
					var path = $(this).val();
					path = path.split("\\");
